...
export function TableOfContents() {
  ...
  const activeLabel =
    TERMS_SECTIONS.find((s) => s.id === activeId)?.tocLabel || "Table of Contents";
      ...
      <div className="block lg:hidden mb-8 sticky top-20 z-30">
        <div className="bg-white/95 backdrop-blur-md rounded-2xl p-4 shadow-md border border-neutral-200">
          ...
          >
            ...
            <ChevronDown
              className={`w-5 h-5 text-neutral-500 transition-transform duration-200 shrink-0 ${isOpenMobile ? "rotate-180" : ""
                }`}
            />
          </button>
              ...
              {TERMS_SECTIONS.map((section) => {
                ...
                return (
                  <button
                    ...
                    className={`text-left px-3 py-2 rounded-lg text-sm transition-colors cursor-pointer ${isActive
            ...
            {TERMS_SECTIONS.map((section) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => scrollToSection(section.id)}
                  className={`w-fit text-left text-[16px] xl:text-[18px] leading-[1.35] tracking-tight transition-all duration-200 cursor-pointer hover:translate-x-1 ${isActive
